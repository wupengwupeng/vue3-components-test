  ...
  <div :id="contentsEdit">
    <div :class="{ isShow: isShow, relative: true }" @click="handlerClickIn">
      <div
        ...
        :id="idd"
        class="border rounded min-h-64 overflow-auto px-15"
      ></div>
      <div
        class="absolute left-15 top-2 z-0 inline-block"
        v-if="isShowPlacehoder"
        @click="handlerClickPlaceHoder"
      >
      ...
      <div
        v-if="isShow"
        id="select-popover"
        @mousedown="handlerClickButnMouseDown"
      >
        <el-popover
          :placement="placementPover"
          :width="200"
          trigger="click"
          :offset="4"
          :show-arrow="false"
        ...
        >
          ...
          <template #default>
            <div class="flex flex-col h-450">
              ...
              <div
                class="flex h-450 w-full flex-col overflow-y-auto"
                @mousedown="handlerClickButnMouseDown"
              >
                <div class="flex flex-col px-6">
                  ...
                  <div
                    class="h-32 text-14 text-gray-700"
                    v-for="(item, index) in allFiledText.taskInfo"
                    :key="index + 'a'"
                  >
                    <el-button
                      size="mini"
                      ...
                      @mousedown="(e) => handlerChange(item.key, item.tag, e)"
                      >{{ item.key }}</el-button
                    >
                  </div>
                </div>
                ...
                <div class="flex flex-col px-6 mt-5 border-t">
                  ...
                  <div
                    class="h-32 text-14 text-gray-700"
                    v-for="(item, index) in allFiledText.testInfo"
                    :key="index + 'b'"
                  >
                    <el-button
                      size="mini"
                      class="w-full"
                      :disabled="item.disabled"
                      @click="handlerChange(item.key, item.tag)"
                      >{{ item.key }}</el-button
                    >
                  </div>
                </div>
                ...
                <div class="flex flex-col px-6 mt-5 border-t">
                  ...
                  <div
                    class="h-32 text-14 text-gray-700"
                    v-for="(item, index) in allFiledText.recordInfo"
                    :key="index + 'g'"
                  >
                    <el-button
                      size="mini"
                ...
                <div class="flex flex-col px-6 mt-5 border-t">
                  ...
                  <div
                    class="h-32 text-14 text-gray-700"
                    v-for="(item, index) in allFiledText.stdInfo"
                    :key="index + 's'"
                  >
                    <el-button
                      size="mini"
  ...
  setup(props, { emit }) {
    ...
    const med = {
      ...
      callBackMethod(content: string) {
        const a = /\{([^{}]+?)\}/g;
        const dic: any[] = [
          ...state.allFiledText.recordInfo,
          ...state.allFiledText.stdInfo,
          ...state.allFiledText.taskInfo,
          ...state.allFiledText.testInfo,
        ];
