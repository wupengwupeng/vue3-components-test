  ...
  >
    <el-tooltip append-to-body placement="top" :show-after="500">
      ...
      <template v-if="modelValue">
        <el-image
          v-if="isPreviewableImageUrl(modelValue)"
        ...
        >
          <img
            v-if="isPDFUrl(modelValue)"
            class="w-48 h-48 object-contain"
            src="@/assets/img/pdf.svg"
            alt
          />
          <img
            v-else-if="isDOCUrl(modelValue)"
            class="w-48 h-48 object-contain"
      ...
      >
        ...
        <div class="w-64 h-64 flex items-center justify-center">
          <app-svg-icon
            class="w-24 h-24"
            icon-name="icon-cloud-upload"
          ></app-svg-icon>
        </div>
        <div class="flex-1 flex flex-col items-start justify-center">
          <div class="h-24 flex items-center">
            <span class="text-14 hover:cursor-pointer mr-5 upload-color-text"
              >点击上传</span
            >
            <span class="text-gray-700 text-14"> 或将文件拖到此处</span>
          </div>
          <div class="h-24 text-gray-500 text-12 flex items-center">
            支持格式:.jpg.png.单个文件不超过20MB
          </div>
        </div>
      </div>
    </el-tooltip>
